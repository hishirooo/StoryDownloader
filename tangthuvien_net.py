...
TOC_ONE_SHOT_PATH = "/story/chapters?story_id={sid}&chapter_id={cid}"
...
def _fetch_full_toc_via_api(base_url: str, story_id: str, chapter_id: str, sess: requests.Session) -> List[Dict]:
    """Tải toàn bộ danh sách chương bằng API, sử dụng cả story_id và chapter_id."""
    last_err = None
    for host in MIRRORS:
        url = f"https://{host}{TOC_ONE_SHOT_PATH.format(sid=story_id, cid=chapter_id)}"
        try:
            ...
            if items:
                print(f"✓ Lấy mục lục thành công qua API, tìm thấy {len(items)} chương.")
                return items
...
def _extract_chapters(soup: BeautifulSoup, base_url: str) -> Tuple[List[Dict], Dict[str, Optional[int]]]:
    ...
    if story_id and chapter_id:
        try:
            chapters = _fetch_full_toc_via_api(base_url, story_id, chapter_id, sess)
        except Exception as e:
            print(f"⚠️ Không thể lấy danh sách chương qua API: {e}")
            print("→ Chuyển sang lấy các chương có sẵn trên trang đầu tiên.")
            pass
    else:
        print("Không tìm thấy đủ story_id và chapter_id để dùng API.")
    ...
    print(f"🔗 Collected chapter links: {len(unique)} (Method: {'API' if story_id and chapter_id and chapters else 'HTML Only'})")
